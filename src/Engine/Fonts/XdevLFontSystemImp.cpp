...
namespace soan {
    ...
    XdevLFont* XdevLFontSystemImpl::createFromFontFile(const xdl::xdl_char* fontInfoFilename) {
        ...
        XdevLFontImpl* font = new XdevLFontImpl(screenWidth, screenHeight, openGL);
        ...
        if(infile.is_open()) {
            ...
            calculateGlyphInformation(font, texture->getWidth(), infile);
    ...
    void XdevLFontSystemImpl::calculateGlyphInformation(XdevLFontImpl* font, xdl::xdl_int textureWidth, std::ifstream& os) {

		xdl::xdl_float numCols 			= (xdl::xdl_float)textureWidth/font->getFontSize();
		xdl::xdl_uint numberOfSegments 	= pow(textureWidth/(xdl::xdl_uint)font->getFontSize(),2);
